...
namespace Xorva.Modules.Platform.Common;

/// <summary>Helpers shared by the Platform (dynamic-entity) handlers.</summary>
public static class PlatformSupport
{
    /// <summary>Turns a human label into a stable machine key ("Course Name" → "course_name").</summary>
    ...
    /// <summary>
    /// Validates a record's submitted values against its field definitions and returns the
    /// canonical JSON to persist (only known field keys, in field order). Throws
    /// <see cref="BadRequestException"/> listing every problem when validation fails.
    /// </summary>
    public static string BuildRecordJson(
        IReadOnlyList<FieldDefinition> fields, IReadOnlyDictionary<string, JsonElement> input)
    {
    ...
    private static bool TryCoerce(FieldDefinition f, JsonElement raw, out object? value, out string? error)
    {
