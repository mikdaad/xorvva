  ...
  return (
    <AppShell>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-frost">Debit Notes</h1>
              ...
              <tbody className="divide-y divide-border">
                {rows.map((d) => (
                  <tr key={d.id} className="hover:bg-hover">
                    <td className="px-4 py-2.5 font-mono text-xs text-frost">{d.number}</td>
                    <td className="px-4 py-2.5 text-frost-dim">{d.contactName}</td>
                    <td className="px-4 py-2.5 text-frost-dim">{new Date(d.date).toLocaleDateString()}</td>
                    <td className="px-4 py-2.5 text-right font-mono tabular-nums text-frost-dim">{money(d.total)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
