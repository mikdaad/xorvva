...
import { ArrowRight, Mail, Gem, Lock, Eye, EyeOff, ArrowLeft, X, AlertCircle, PartyPopper, Loader, Sparkles } from "lucide-react";
  ...
  >
    <defs>
      <linearGradient id="rev_grad1" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop
          offset="0%"
          style={{ stopColor: "#448E48", stopOpacity: 0.8 }}
        />
        <stop
          offset="100%"
          style={{ stopColor: "#AEAEAE", stopOpacity: 0.6 }}
        />
      </linearGradient>
      ...
      <linearGradient id="rev_grad2" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop
          offset="0%"
          style={{ stopColor: "#AEAEAE", stopOpacity: 0.9 }}
        />
        <stop
          offset="50%"
          style={{ stopColor: "#FFFFFF", stopOpacity: 0.7 }}
        />
        <stop
          offset="100%"
          style={{ stopColor: "#448E48", stopOpacity: 0.6 }}
        />
      </linearGradient>
    ...
    {/* Bottom Left Green/Grey Ellipse */}
    <motion.ellipse
      cx="200"
      cy="500"
      rx="250"
    ...
    {/* Top Right Grey/White/Green Rect */}
    <motion.rect
      x="500"
      y="100"
      width="300"
...
const modalSteps = [
  ...
  { message: "Preparing ledger...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
...
export const LoginComponent = ({ logo = <DefaultLogo />, brandName = "TrueLedge" }: LoginComponentProps) => {
  const router = useRouter();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [authStep, setAuthStep] = useState<"email" | "password">("email");
  const [modalStatus, setModalStatus] = useState<'closed' | 'loading' | 'error' | 'success' | 'magicLinkSent'>('closed');
  ...
  const handleMagicLink = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setModalStatus('magicLinkSent');
      }
    } catch (err: any) {
      setModalErrorMessage(err?.message || "Magic Link request failed.");
      setModalStatus('error');
    }
  };
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setModalErrorMessage(err?.message || "Sign in failed.");
      setModalStatus('error');
    }
  };
      ...
      {modalStatus !== 'closed' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-card/80 border-4 border-border rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-2">
            {(modalStatus === 'error' || modalStatus === 'success' || modalStatus === 'magicLinkSent') && <button onClick={closeModal} className="absolute top-2 right-2 p-1 text-muted-foreground hover:text-foreground transition-colors"><X className="w-5 h-5" /></button>}
            {modalStatus === 'error' && <>
            ...
            {modalStatus === 'magicLinkSent' &&
              <div className="flex flex-col items-center gap-4 text-center">
                <Sparkles className="w-12 h-12 text-sky-400 animate-bounce" />
                <p className="text-lg font-medium text-foreground">Magic Link Sent!</p>
                <p className="text-sm text-muted-foreground">Check your email inbox for a magic link to sign in instantly.</p>
                <GlassButton className="mt-2" onClick={closeModal} size="sm">Got it</GlassButton>
              </div>
            }
          </motion.div>
        </motion.div>
        ...
        <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
          <AnimatePresence mode="wait">
            {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
              <BlurFade className="w-full" delay={0.1}><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl md:text-6xl tracking-tight text-foreground whitespace-nowrap">Welcome back</p></div></BlurFade>
              <BlurFade delay={0.15}><p className="text-sm font-medium text-muted-foreground">Sign in with</p></BlurFade>
            ...
            {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
              <BlurFade className="w-full" delay={0}><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">Enter Password</p></div></BlurFade>
              <BlurFade delay={0.15}><p className="text-sm font-medium text-muted-foreground">Welcome back, enter your password to sign in.</p></BlurFade>
            </motion.div>}
          </AnimatePresence>

          <form onSubmit={handleFinalSubmit} className="w-[300px] space-y-6 flex flex-col items-center">
            <div className="w-full space-y-6">
              {authStep === 'email' && (
                <BlurFade className="w-full" delay={0.1} inView={true}>
                ...
                <BlurFade className="w-full" key="password-field">
                  <div className="relative w-full">
                    ...
                    <div className="glass-input-wrap w-full"><div className="glass-input">
                      ...
                      <div className={cn("relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isPasswordValid ? "w-10 pr-1" : "w-0")}><GlassButton aria-label="Submit password" contentClassName="text-foreground/80 hover:text-foreground" size="icon" type="submit"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                    </div></div>
                  </div>
            ...
            {authStep === 'email' && isEmailValid && (
              <BlurFade delay={0.3} className="w-full flex flex-col items-center gap-2">
                <button type="button" onClick={handleMagicLink} className="text-xs text-sky-400 hover:text-sky-300 transition-colors underline underline-offset-4 cursor-pointer">
                  Send Magic Link to Email
                </button>
              </BlurFade>
            )}

            <BlurFade delay={0.35} className="w-full text-center pt-2">
